...
def test_basic_functionality():
    ...
    try:
        ...
        if encrypted_data:
            print("SUCCESS: Encryption successful!")
        ...
        if decrypted_message:
            print("SUCCESS: Decryption without password successful!")
            print(f"Decrypted message: {decrypted_message}")
        else:
            print("ERROR: Decryption without password failed!")
            return False
        ...
        if decrypted_message and decrypted_message == test_message:
            print("SUCCESS: Decryption with password successful!")
            print(f"Decrypted message: {decrypted_message}")
            print("SUCCESS: Message verification successful!")
            return True
        else:
            print("ERROR: Decryption with password failed!")
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            if response.status_code == 200:
                ...
                print(f"ERROR: Dashboard route failed: {response.status_code}")

            # Test encryption endpoint (without file)
            response = client.post('/encrypt')
            if response.status_code == 400:
                print("OK: Encryption endpoint responds (expected 400 without file)")
            else:
                print(f"WARNING: Encryption endpoint unexpected response: {response.status_code}")

        return True
    except Exception as e:
        print(f"ERROR: Flask routes test failed: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if imports_ok:
        ...
        if basic_ok and flask_ok:
            print("\nSUCCESS: All tests passed! CyberCloak is working correctly.")
        else:
            print("\nFAILURE: Some tests failed. Please check the implementation.")
    else:
        print("\nFAILURE: Import tests failed. Please check dependencies.")
